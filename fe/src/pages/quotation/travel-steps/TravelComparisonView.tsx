...
const TravelComparisonView: React.FC<Props> = ({ formData, onPrev }) => {
  ...
  useEffect(() => {
    const fetchQuotes = async () => {
      try {
        setLoading(true);
        const response = await api.post('/quotations/travel/compare', formData);
  ...
  const handleSelectQuote = (quote: InsurerQuote) => {
    // In real app: create quotation then navigate to purchase
    navigate(`/purchase?insurance_type=travel`);
  };
  ...
  if (loading) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-600">Đang tìm báo giá tốt nhất cho bạn...</p>
        <p className="text-sm text-gray-400 mt-1">Đang liên hệ các nhà bảo hiểm</p>
      </div>
    );
  }
  ...
  return (
    <div>
      <h2 className="text-xl font-semibold text-gray-900 mb-2">So Sánh Báo Giá</h2>
      <p className="text-gray-600 mb-6">
        Tìm thấy {quotes.length} báo giá cho chuyến đi của bạn
      </p>
      ...
      <div className="bg-blue-50 rounded-lg p-4 mb-6">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
          <div>
            ...
            <span className="ml-1 font-medium">{formData.destination_country || formData.destination_type}</span>
          </div>
          <div>
            <span className="text-gray-500">Ngày đi:</span>
            <span className="ml-1 font-medium">{formData.departure_date}</span>
          </div>
          <div>
            <span className="text-gray-500">Ngày về:</span>
            <span className="ml-1 font-medium">{formData.return_date}</span>
          </div>
          <div>
            <span className="text-gray-500">Số người:</span>
            <span className="ml-1 font-medium">{formData.travelers.length}</span>
          </div>
        </div>
      </div>
          ...
          <div key={index} className="border rounded-xl p-5 hover:shadow-md transition-shadow">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              ...
              <div className="text-right">
                ...
                <p className="text-sm text-gray-500">
                  ~{formatCurrency(Math.round(quote.premium.total / formData.travelers.length))}/người
                </p>
                <button
            ...
            <div className="mt-4 pt-4 border-t">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                {quote.coverage_details.slice(0, 4).map((c, i) => (
      ...
      {quotes.length === 0 && !loading && (
        <div className="text-center py-8 text-gray-500">
          <p>Không tìm thấy báo giá phù hợp. Vui lòng thay đổi điều kiện.</p>
        </div>
      )}
...
export default TravelComparisonView;
